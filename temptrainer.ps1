...
# NOTE: do NOT upgrade/install mlagents here to avoid changing pinned deps in the export venv.
# If you really need to install mlagents in this venv, uncomment the next two lines.
...
if (Test-Path $mlExe) {
    ...
    & $mlExe ".\Assets\training\trainer_config.yaml" --run-id "FighterRun14" --force
} else {
    Write-Output "mlagents-learn.exe not found, falling back to python -m mlagents_learn"
    & $python -m mlagents_learn ".\Assets\training\trainer_config.yaml" --run-id "FighterRun14" --force
}